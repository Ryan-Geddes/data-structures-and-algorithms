...
copy the contents, fill it with the Star Wars People, and append it to the DOM.
------------------------------------------------------------------------------------------------ */
let starWarsPeople = [
  {
    "name": "Luke Skywalker",
...
Your function should use slice within a loop and return an 
array of each successive string result from losing letters 
...
const howMuchPencil = (str) => {
  ...
  for(let i = 0; i <arr.length; i++){
        let sliceArr = arr.slice(i,arr.length);
        let pencil = sliceArr.join('');
        result.push(pencil);
  }
  result.push('');
...
given a string as input, returns a new array where every element 
...
const listFoods = (recipe) => {
    ...
    for (let i =0; i <ingArr.length; i++){
          let regex = /^((?:\S+\s+){2})/g;
          let checker = ingArr[i].match(regex);
          result.push(ingArr[i].slice((checker.join('').length),ingArr[i].length));
    }
    return result;
  };
...
//   ingArr.forEach(item => {
//       let regex = /^((?:\S+\s+){2})/g;
//       let checker = item.match(regex);
//       result.push(item.slice((checker.join('').length),item.length));
//         if (result.length >= 10){
//             return result;
//         }
//   });
