...
Write a function named templatingWithMustache that uses mustache to create the markup templates for each of the characters. 
Use the snippet as your guide for creating your templates. Return an array of template strings. 
...
const templatingWithMustache = () => {
  ...
  characters.forEach( person =>{
        let temp = Mustache.render($template, person);
        output.push(temp);
  })
  return output;
};
...
Write a function named getHouses that returns a new array 
containing the names of all of the houses in the data set.
------------------------------------------------------------------------------------------------ */

const getHouses = (arr) => {
  let houses = [];
  arr.forEach( person =>{
    houses.push(Object.values(person)[3]);
    })
//   });
  return houses;
};
...
const hasChildrenEntries = (arr, character) => {
  arr.forEach(function(person, idx){
    if (Object.entries(person)[2][1].length > 0){
      console.log('TRUE', person.name)
      return true;
    }
    else{
      console.log('FALSE', person.name)
      return false;
    }
