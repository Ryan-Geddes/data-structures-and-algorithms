...
const count = (target, input) => {
  let counter = input.reduce ( function(acc, arr, idx){
...
are not numbers or are not divisible by five.
...
const divisibleByFiveTwoToThePower = (input) => {
        ...
        input.forEach ((arr, idx) =>{
            let checker = arr.filter( (val, idx)=>{
              return !(isNaN(val)) && !(val % 5) && (typeof val)!='string';
            });
          output.push(checker);
        ...
        let output2 = [];
        output.forEach ((arr, idx)=>{
          let mather = arr.map((val,idx)=>{
